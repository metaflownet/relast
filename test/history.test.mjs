...
test('resolveCloseRestorePlan accepts Safari multi-second delayed close activation', () => {
  const restorePlan = resolveCloseRestorePlan({
    windowHistory: {
      '1': [40, 30, 20, 10],
    },
    ...
    removalTime: 6106,
    transientActivationWindowMs: 10000,
  });

  assert.equal(restorePlan.removedWasMostRecent, true);
  assert.equal(restorePlan.usedTransientActivation, true);
  assert.equal(restorePlan.restoreTargetTabId, 20);
});
