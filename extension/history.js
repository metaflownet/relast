...
function replaceTabInTabIdList(
  rawTabIds,
  removedTabId,
  addedTabId,
...
function normalizeWindowHistory(rawWindowHistory, maxHistorySize = MAX_HISTORY_SIZE) {
...
function setWindowHistory(windowHistory, windowId, tabIds, maxHistorySize = MAX_HISTORY_SIZE) {
...
function addToWindowHistory(windowHistory, windowId, tabId, maxHistorySize = MAX_HISTORY_SIZE) {
...
function getWindowHistory(windowHistory, windowId) {
...
function removeTabFromAllWindowHistories(windowHistory, tabId) {
...
function replaceTabInAllWindowHistories(windowHistory, removedTabId, addedTabId) {
  if (!isValidTabId(removedTabId) || !isValidTabId(addedTabId)) {
    return normalizeWindowHistory(windowHistory, MAX_HISTORY_SIZE);
  }
...
function removeWindowHistory(windowHistory, windowId) {
  const nextHistory = normalizeWindowHistory(windowHistory, MAX_HISTORY_SIZE);
  delete nextHistory[String(windowId)];
  return nextHistory;
}

function pruneWindowHistory(windowHistory, openTabsById, maxHistorySize = MAX_HISTORY_SIZE) {
...
function normalizeTabMetadata(rawTabMetadata) {
...
function upsertTabMetadata(tabMetadata, tab) {
...
function removeTabMetadata(tabMetadata, tabId) {
  const nextTabMetadata = normalizeTabMetadata(tabMetadata);
  delete nextTabMetadata[String(tabId)];
  return nextTabMetadata;
}

function replaceTabMetadata(tabMetadata, removedTabId, addedTabId, addedTab = null) {
  const nextTabMetadata = normalizeTabMetadata(tabMetadata);
  if (!isValidTabId(removedTabId) || !isValidTabId(addedTabId)) {
    return nextTabMetadata;
  }
...
function removeWindowTabMetadata(tabMetadata, windowId) {
  const nextTabMetadata = normalizeTabMetadata(tabMetadata);
...
function pruneTabMetadata(tabMetadata, openTabsById) {
...
function normalizeActivationByWindow(rawActivationByWindow) {
  if (
...
function setActivationRecord(lastActivationByWindow, windowId, record) {
...
function removeTabFromActivationByWindow(lastActivationByWindow, tabId) {
  const nextActivationByWindow = normalizeActivationByWindow(lastActivationByWindow);
...
function replaceTabInActivationByWindow(lastActivationByWindow, removedTabId, addedTabId) {
  if (!isValidTabId(removedTabId) || !isValidTabId(addedTabId)) {
...
function removeWindowActivation(lastActivationByWindow, windowId) {
  const nextActivationByWindow = normalizeActivationByWindow(lastActivationByWindow);
  delete nextActivationByWindow[String(windowId)];
  return nextActivationByWindow;
}

function moveTabInActivationByWindow(lastActivationByWindow, tabId, windowId) {
...
function pruneActivationByWindow(lastActivationByWindow, openTabsById) {
...
function resolveCloseRestorePlan({
  windowHistory,
  windowId,
  removedTabId,
...
function resolvePendingRestoreActivation({
  windowHistory,
  lastActivationByWindow,
  windowId,
  activatedTabId,
